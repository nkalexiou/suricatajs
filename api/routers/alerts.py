...
def approve_alert(alert_id: int, current_user: dict = Depends(get_current_user)):
    ...
    with get_connection() as conn:
        row = conn.execute(
            text("SELECT id, javascript, new_checksum, alert_type FROM alerts WHERE id = :id"),
            {"id": alert_id},
        ).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Alert not found")
        script_uri, new_checksum, alert_type = row[1], row[2], row[3]
        # For checksum-mismatch alerts: update the stored baseline to the new hash.
        # For new-script alerts: the baseline was already written by the scanner;
        # approving just marks the alert resolved without touching the baseline.
        if alert_type == "checksum":
            if not new_checksum:
